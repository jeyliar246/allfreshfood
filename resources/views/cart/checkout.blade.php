<x-home-layout>
                        ...
                        <div class="card-body">
                            ...
                            @auth
                                <div class="mb-3">
                                    ...
                                    <input type="tel" name="phone" id="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="Enter your phone number" value="{{ old('phone', optional(Auth::user())->phone) }}">
                                    @error('phone')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            @guest
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <input type="tel" name="guest_phone" id="guest_phone" class="form-control @error('guest_phone') is-invalid @enderror" value="{{ old('guest_phone') }}" required>
                ...
                <div class="card sticky-top" style="top: 1rem;">
                    ...
                    <div class="card-body">
                        ...
                        @php
                            ...
                            $flatDelivery = $isPickup ? 0.00 : (float) ($deliveryAmount ?? 0);
                        @endphp
                        ...
                        <div class="d-flex justify-content-between mb-2" id="delivery_fee_row" style="display: {{ old('fulfillment_method') === 'pickup' ? 'none' : 'flex' }};">
                            <span>Delivery</span>
                            <span>£<span id="delivery-amount">{{ number_format($flatDelivery, 2) }}</span></span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between fw-bold fs-5 mb-4">
                            <span>Total</span>
                            <span>£<span id="total-amount">{{ number_format(floatval(str_replace(',', '', $subtotal)) + $flatDelivery + floatval(str_replace(',', '', $tax)), 2) }}</span></span>
                        </div>
    ...
    <script>
        ...
        // Live compute delivery fee and totals based on pickup/delivery toggle
        (function() {
            const subtotal = parseFloat(("{{ $subtotal }}").replace(/,/g, '')) || 0;
            const tax = parseFloat(("{{ $tax }}").replace(/,/g, '')) || 0;
            const configuredDelivery = parseFloat(("{{ $deliveryAmount ?? 0 }}").replace(/,/g, '')) || 0;
            ...
            function updateTotals() {
                ...
                let delivery = isPickup ? 0 : configuredDelivery;
