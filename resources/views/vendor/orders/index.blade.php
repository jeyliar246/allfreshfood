    ...
    <div class="main-content">
        ...
        <div class="container-fluid p-4">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @forelse($orders as $order)
                                    {{-- Reset the total for each specific order here --}}
                                    @php
                                        $orderVendorTotal = 0;
                                    @endphp
                                    ...
                                    @foreach ($order->items as $item)
                                        @php
                                            $orderVendorTotal += $item->price * $item->quantity;
                                        @endphp
                                    @endforeach
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $order->user->name ?? '' }} @if ($order->guest_name)
                                                {{ $order->guest_name }}
                                            @endif
                                        </td>
                                        <td>{{ number_format($order->total, 2) }} </td>
                                        <td>{{ number_format($orderVendorTotal, 2) }}</td> 
                                        <td>{{ ucfirst($order->status) }}</td>
                                        <td>
                                            <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                                data-bs-target="#vendorOrderModal{{ $order->id }}">View</button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
            ...
            @foreach ($orders as $order)
                ...
                <div class="modal fade" id="vendorOrderModal{{ $order->id }}" tabindex="-1"
                    aria-labelledby="vendorOrderModalLabel{{ $order->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-lg modal-dialog-scrollable">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="vendorOrderModalLabel{{ $order->id }}">Order
                                    #{{ $order->id }} Details</h5>
                            ...
                            <div class="modal-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        <p class="mb-1"><strong>Customer:</strong> {{ $order->user->name ?? '' }}
                                            @if ($order->guest_name)
                                                {{ $order->guest_name }}
                                            @endif
                                        </p>
                                    ...
                                    <div class="col-md-6">
                                        <p class="mb-1">
                                            <strong>Phone:</strong><br />{{ $order->user->phone ?? $order->guest_phone }}
                                        </p>
                                        ...
                                        <p class="mb-1"><strong>Address:</strong>
                                            {{ $order->user->address ?? $order->delivery_address }}</p>
                                    </div>
                                </div>
                                <hr />
                                <h6>Items</h6>
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="fw-bold">
                                                <td colspan="3" class="text-end">Vendor's Total:</td>
                                ...
                                <form action="{{ route('vendor.orders.updateStatus', $order) }}" method="POST"
                                    ...
                                    <select name="status" class="form-select form-select-sm">
                                        ...
                                        @foreach ($statuses as $st)
                                            <option value="{{ $st }}"
                                                {{ $order->status === $st ? 'selected' : '' }}>{{ ucfirst($st) }}
                                            </option>
                                        @endforeach
                                    </select>
