...
class VendorFinanceController extends Controller
{
    public function index()
    {
        ...
        // $deliveredTotall = Order::where('vendor_id', $vendor->id)
        ...
        //pending orders
         ...
         $porders = Order::where('vendor_id', $vendor->id)
            ->where('status', '!=', 'delivered')
            ->get();
        ...
        foreach ($porders as $order) {
            foreach ($order->items as $item) {
                $tott += $item->product->price * $item->quantity;
            }
        }

        $pendingTotal = $tott;

        return view('vendor.finance.index', compact('vendor', 'balance', 'pendingPayouts', 'deliveredTotal', 'payoutTotal', 'pendingTotal'));
    }
